...
import { LucideAngularModule, Plus, Edit2, Trash2, X, Check, Award, Crown, Star, Shield } from 'lucide-angular';
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, LucideAngularModule , CountLevelPipe],
...
export class AdminFidelityComponent implements OnInit {
  ...
  submit(): void {
    if (this.isEditing()) {
      this.service.update(this.editingId()!, this.form).subscribe({
        next: () => { this.closeModal(); this.loadAll(); this.showSuccess('Fidelity modifiée !'); },
        error: () => { this.errorMessage = 'Erreur lors de la modification.'; }
      });
    } else {
      this.service.create(this.form).subscribe({
        next: () => { this.closeModal(); this.loadAll(); this.showSuccess('Fidelity créée !'); },
  ...
  delete(id: string): void {
    if (!confirm('Supprimer cette fidelity ?')) return;
    this.service.delete(id).subscribe({
      next: () => { this.fidelities = this.fidelities.filter(f => f.id !== id); this.showSuccess('Fidelity supprimée !'); },
  ...
  totalPoints(): number {
    return this.fidelities.reduce((sum, f) => sum + f.points, 0);
  }
}